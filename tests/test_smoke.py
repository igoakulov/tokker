...
class SmokeTests(unittest.TestCase):
    def test_models_list(self):
        ...
        self.assertIn("HuggingFace (BYOM - Bring You Own Model):", stdout, "Models output missing HuggingFace section")
        openai_idx = stdout.index("OpenAI:")
        google_idx = stdout.index("Google:")
        hf_idx = stdout.index("HuggingFace (BYOM - Bring You Own Model):")
        self.assertTrue(openai_idx < google_idx < hf_idx, "Providers not in expected order (OpenAI, Google, HuggingFace)")
    ...
    def test_tiktoken_model(self):
        ...
        expected_fields = ["delimited_text", "token_strings", "token_ids", "token_count", "word_count", "char_count"]
        for field in expected_fields:
            self.assertIn(field, result, f"Missing field '{field}' in tiktoken result")
        self.assertEqual(result["token_count"], len(result["token_strings"]), "Token count mismatch")

    @unittest.skipUnless(_has_transformers(), "Skipping: transformers (hf) extra not installed")
    def test_huggingface_model(self):
        ...
        expected_fields = ["delimited_text", "token_strings", "token_ids", "token_count", "word_count", "char_count"]
        for field in expected_fields:
            self.assertIn(field, result, f"Missing field '{field}' in HuggingFace result")
        self.assertEqual(result["token_count"], len(result["token_strings"]), "Token count mismatch")
    ...
    def test_output_formats(self):
        ...
        self.assertTrue(isinstance(pivot, dict) and bool(pivot), "Pivot should be a non-empty object")
        self.assertTrue(("foo" in pivot) or (" foo" in pivot), "Pivot missing expected token 'foo'")
    ...
    def test_model_default(self):
        ...
        self.assertIn("Default model set to: cl100k_base", stdout, "Default model confirmation missing")
    ...
    def test_error_handling(self):
        ...
        self.assertNotEqual(exit_code, 0, "Invalid default model should have failed")
    ...
    def test_history_functionality(self):
        ...
        self.assertTrue(("History is already empty." in stdout) or ("cleared" in stdout.lower()), "History clear confirmation missing")

        exit_code, stdout, stderr = run_command([
            "tok", "--history"
        ])
        self.assertEqual(exit_code, 0, f"History check after clear failed: {stderr}")
        self.assertTrue(("History empty." in stdout) or ("Your history is empty" in stdout) or ("Your history is empty" in stderr), "History not cleared properly")
